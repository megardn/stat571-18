...
# while loops can be slow in R, I'll use one here because we don't have that many teams
...
p_plot <- ggplot(data=teamwise_regression_result, aes(x=team,y=p_value, fill=team)) + geom_bar(stat='identity') +
  ...
  ggtitle("Teamwise ∆Win Percentage ~ ∆Log Payroll Regression p-value") + theme(legend.position = "none")
...
rsq_plot <- ggplot(data=teamwise_regression_result, aes(x=team,y=effect_size, fill=team)) + geom_bar(stat='identity') +
  theme(axis.text.x = element_text(angle = -90)) +
  ggtitle("Teamwise ∆Win Percentage ~ ∆Log Payroll Regression Effect Size (Adj. R^2)") + theme(legend.position = "none")
gridExtra::grid.arrange(p_plot, rsq_plot, nrow=1) # plot these together

```
We can thus infer than this predictive relationship between change in log payroll and change in win percentage is only marked in a handful of teams. Specifically, the following teams, whose corresponding regressions resulted in a p-value less than 0.05, with substantial Adj. R^2 values:
