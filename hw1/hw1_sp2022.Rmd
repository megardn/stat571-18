...
``` {r c1 load mturk}
mturk <- read.csv("data/Survey_results_final.csv", header = TRUE, stringsAsFactors = F, na = "") #read data into df from csv and replacing blanks with NA
colnames(mturk)
```
...
``` {r c1 drop na}
radio <- radio %>% drop_na(sirius) %>% drop_na(wharton) #drop any rows missing sirius/wharton listenership, since that'ss what we care about most
```
...
``` {r c1 missing&incorrect}
parse_number(radio$age) #find variables in age with no numerical component
radio <- radio %>% mutate(age = parse_number(age, na="female")) #replace identified chr variables with NA and recover entries with chr & int data
...
#listing unique levels to make sure all make sense - need to reorder income & education?
radio %>% sapply(levels)

#clean up education
is.na(radio$education) <- which(radio$education == "select one") #replace unselected education values with NA
radio$education <-droplevels(radio$education) #drop the now unused "select one" level
...
n <- nrow(radio) #sample size
n
(sum(!complete.cases(radio))/n)*100 #percent of subj missing one or more responses

sapply(radio, function(x) sum(!is.na(x))) #sample size for each variable
sapply(radio, summary) #summary of all data
```
...
(wlisten/n)*100 #percent subj who listen to wharton
(wlisten/slisten)*100 #percent subj listening to sirius who listen to wharton
```
