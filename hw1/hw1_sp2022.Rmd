...
```{r plotting proportions, echo=FALSE}
...
w_df %>% group_by(Field, Sex) %>%
  summarise(total_number = sum(Number)) %>%
  # bar plots per gender and field type (while sorting data by max number in males per field)
  ggplot(aes(x = Field, y = total_number, fill = Sex, colour = Sex)) + 
    geom_bar(stat = "identity", position = "fill", alpha = 0.8, width = 0.5) +
    theme_bw() +
    labs(x="Fields of study", y = "Total number") +
    ggtitle("Number of awarded degrees by field type and gender") +
...
# probably better to make some kind of a difference measure or cumulative measure
# summarizing by field and sex
agg <- w_df %>% 
...
Based on the above, we can observe that gender proportions tend to remain table across the years (2006-2016), regardless of field type (`Non-S&E` vs `Science fields`) or subfields.  
