---
title: " Modern Data Mining, HW 1 - Group 18"
author:
- Joelle Bagautdinova
- Diego G. Davila
- Margaret Gardner
...
Usable data was obtained from `r nrow(radio)` subjects. Of these, `r (sum(!complete.cases(radio))/nrow(radio))*100`% of subject's data was incomplete (i.e. missing one or more demographic variables). The number of usable responses for each variable is shown below:
...
The distribution of respondents' ages is plotted below. Respondents ages ranged from `r min(radio$age, na.rm=T)` to `r max(radio$age, na.rm=T)` years with a mean of `r mean(radio$age, na.rm=T)` years. 
``` {r age.plot}
...
The distribution of the time taken by each respondents to complete the survey is plotted below. Respondents time to completion ranged from `r min(radio$worktime, na.rm=T)` to `r max(radio$worktime, na.rm=T)` seconds with a mean of `r mean(radio$worktime, na.rm=T)` seconds. 
``` {r worktime.plot}
...
```{r mturk.education, fig.height=8}
#collapse variable levels to match sample
