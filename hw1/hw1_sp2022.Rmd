...
``` {r c1 load mturk}
mturk <- read.csv("data/Survey_results_final.csv", header = TRUE, stringsAsFactors = F, na = "") #read data into df from csv and replacing blanks with NA ***strings as factors?
colnames(mturk)
```
...
``` {r}
radio <- radio %>% drop_na(sirius) %>% drop_na(worktime) #drop any rows missing sirius/wharton listenership, since this is what we care about most
```
...
``` {r c1}
parse_number(radio$age) #find variables in age with no numerical values
...
#now that age is numeric, find min/max and drop data outside reasonable age range:
