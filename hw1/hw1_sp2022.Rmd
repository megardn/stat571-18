...
``` {r c1 drop na}
radio <- radio %>% drop_na(sirius) %>% drop_na(wharton) #drop any rows missing sirius/wharton listenership, since this is what we care about most
```
...
radio <- radio %>% mutate(age = na_if(age, "female")) %>% mutate(age = parse_number(age)) #replace identified chr variables with NA and recover entries with chr & int data
...
```
``` {r education}
d<- ggplot(radio, aes(x=education)) + geom_bar(stat="count") #should i drop NA before plotting?
d
```
