...
pacman::p_load(randomForest, tree, ISLR, rpart, rattle, pROC, partykit, ggplot2, glmnet, leaps, dplyr, keras, neuralnet, imager, ranger, tm, GGally, RColorBrewer, wordcloud, parallel, MASS, doParallel, tidyselect, report)
```
...
```
The reviews were collected between Oct. 19th, 2004 and Oct. 4th, 2018. 
...
The sparsity of the matrix is 97% here, meaning that 97% of the elements are zero. Thsi means that 97% of the time a given word does not appear in a given document. 
...
Among all the words used in the sampled reviews, "thorough" and "delight" are most predictive of a high rating. For each time that "thorough" or "delight" are used in a review, the log(odds) of the rating being 4 or 5 stars increases by about 2.5487 and 1.6607, respectively. 
...
```
Each word displayed is predictive of a higher rating (4 or 5 stars), with size corresponding to the strength of that association. Many of these word stems ("pleasur", "delight", "comfort", "goto", etc.) have strong positive connotations. However, some stems, like "slight", "thursday", or "worri", are less intuitively positive, indicating that the associations identified by the model implicitly account for the context in which the stem appears. In other words, the model is able to identify "worri" as predicting positive ratings, suggesting that many positive reviews include phrases such as "no worries", "nevery worry" or similar. 
...
Out of all words stems to appear in the reviews, "wors" and "rude" are most predictive of a low (3 or fewer star) rating. For each time that "wors" or "rude" are used in a review, the log(odds) of the rating being 4 or 5 stars decreases by about 2.4189 and 2.1739, respectively. 
...
mean(data2.test$rating != predict.lasso) #testing error - i think this is what they're asking for?
```
...
Random Forest methods consist of leveraging multiple decision trees to solve classification or regression problems. Some number of decision trees are initialized, each using a subset of the available variables, and a bootstrapped sampled of the data. In the case of a regression problem, the aggregated output is typically an average of all predictions based on each decision tree. In the case of a classification problem, the aggregated output is typically a majority vote, based on the classification by each decision tree. - [FEEL FREE TO EXPAND]
...
```
The testing error of majority vote is: `test.data.error`. 
...
#save(pc.terms, file = "data/terms_prcomp.Rdata") too big! pair down and resave so it'll fit on github
```

```{r}
#load("data/terms_prcomp.Rdata")
...
Using 100 PCs based on scree plot.
...
mean(data2.test$rating != predict.pc.lasso) #testing error
```
...
The prediction error of the ensemble of models is `ensemble.prediction.error`. This is far worse than our best fitting model (LASSO). 
...
The model that seems to produce the lowest testing error is the LASSO model. This is not terribly surprising, as LASSO selects the most important words to use in making its prediction. The performance of LASSO is not that far off from Random Forest's, though it is a bit surprising that RF did not perform better. Perhaps using many more trees would improve our performance to the extent that it surpasses LASSO. It is also not terribly surprising that applying PCA first in fact only makes the prediction slightly worse, since PCA needs almost as many components as variables to explain a reasonable amount of variance. It seems PCA may not be well suited for this particular data. The poor performance of the ensemble was surprising, which may be due to the mixing in of a poor model, or perhaps majority vote implementation. 

Our Final Model: LASSO
```{r}
summary(fit.glm) # same as LASSO with lambda.1se
```


```{r}
lasso.validation.prediction <- predict(fit.lasso, as.matrix(data2.validation[, -1]), type = "class", s="lambda.1se") 
lasso.validation.error <- mean(data2.validation$rating != lasso.validation.prediction)
lasso.validation.error
```

The validation error for our final model (LASSO) is `lasso.validation.error`. 

-FEEL FREE TO EXPAND OR GO INTO DETAIL-
Were we tasked with making a prediction with this model, given a review, we would do the following:
...
5. Feed it as input to the predict() function, using our fit.lasso as the model, with type='class', s='lambda.1se'. 
