...
df <- df %>% 
  ...
  select(-c(Subject, Income2005))

# label last row as Michelle
Michelle <- tail(df, n = 1)
...
Based on the linear models above, the following observations can be made: 
...
# cp = 0.015
fit2.p4 <- prune(fit2, cp = 0.015)  # prune fit2 using prune() from the rpart package (as the tree has been generated with rpart)
print(paste("cp = 0.015: ", round(sum((predict(fit2.p4, data.test) - data.test$log_income)^2), 3))) # testing error 
...
**Answer:** As it turns out, while cp = 0.009 has the smallest training error, cp = 0.015 has the smalles testing error. Thus, let's build the pruned tree using this criterion: 

```{r}
# summary(fit2.pruned)
data.table(fit2.p4$frame)
...
text(fit2) # main tree
plot(fit2.p4)
title(main="Best pruned tree (cp = 0.015)")
text(fit2.p4) # pruned tree
