...
At row 100, column 405, we have a number 0. This corresponds to the amount of times that the word experience (cropped to experienc in the processed matrix) appears in document 100. 
...
data2.train_sub <- data2.train %>% select(all_of(sel_cols)) #subset data
...
plot(rf.fit1, type="p", pch=16,col="green4", main = "Out of Bag MSE")
```

In order to tune the ntree parameter, we have plotted the Out of Bag Mean Squared Error against the number of trees used, with mtry held constant. Based on this plot, 100-150 trees seem to be appropriate, as the decrease in error is negligible. Since the computing power required is not high, we selected to use 150 trees. 

Now, we will focus on optimizing mtry. To do this, we will do randomForest over a range of mtry values, with ntree held constant, and plot the MSE for each value. Because this process will be computationally expensive, I have run my code with parallel processing and saved the output to mse_mtry.Rdata, so as to not have to re-run this chunk. 
...
As we can see, the MSE begins to plateau around 50 variables, so we can select this as our mtry. 

```{r}
rf.fit.final <- randomForest(rating~., data2.train, mtry=50, ntree=150) # fit the final random forest model
