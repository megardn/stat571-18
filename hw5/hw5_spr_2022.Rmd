...
**Answer:** Based on the models above, we note that Gender does have a significant effect on log_income. Specifically, males tend to earn 10.097 + 0.09 more log_income compared to females (reference level).  
...
fit4.final <- randomForest(log_income ~., data.train, mtry = 5, ntree = 200)
plot(fit4.final)
...
fit5 <- (predict(fit1, data.train) + predict(fit2, data.train) + predict(fit3, data.train) + predict(fit4.final, data.train))/4 #bagging
...
# fit3
fit3.mse_oob <- mean((data.test$log_income - predict(fit3, data.test))^2)
...
**Answer:** fit5 has the largest testing error. **CHECK THIS - this seems really odd??**
...
**Answer:** Based on the testing errors shown above, fit4.final is the best model to predict income because it has the smallest testing error. The prediction error using the validation data set is even lower (`r fit4.mse_oob_validation`), confirming that this is a good model for the prediction of log income. **Anything to add to this analysis? Just chose based on smallest error but is there some other thing that needs to be done?**
...
**Answer:** Michelle's predicted log_income is `r fit4.final.Michelle`, which corresponds to $ `r Michelle.fit.4_pred_income`. **This is essentially the same thing as already outputted for fit4.final - is there anything else they want us to do here?**
