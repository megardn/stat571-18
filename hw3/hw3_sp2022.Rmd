...
Next we'll try to account for interaction effects, specifically starting with two possible identified by Anova in Part B. 
...
Based on the Adjusted $R^2$, the explanatory power gained in the interaction models over the main-effects model does not warrant the added complexity and difficulty in interpretation for our purposes. However, it is interesting to consider how acceleration grows (cars take longer to accelerate from 0 to 60 mph) the effect of horsepower on gpm increases; *why would this be happening?*

Let's see what happens when we model sqrt(gpm), since that had the prettiest residuals in the full model.

``` {r}
#cp for all possible d's
fit.sqrt_gpm <- regsubsets(sqrt.gpm ~., auto.sqrt.gpm , nvmax=15, method="exhaustive")
sum_fit.sqrt_gpm <- summary(fit.sqrt_gpm)
sum_fit.sqrt_gpm

#plot
plot(sum_fit.sqrt_gpm$cp, xlab="Number of predictors", 
     ylab="Cp", col="red", pch=16)
```
...
We create a new car for prediction:
