...
```
Based on the elbow rule looking both at CP and BIC, d=3 is sufficient to model gpm. Going to compare d=3 and d=4 solutions to see how adding cylinders (categorical) changes the fit (e.g. is adding all levels of cylinder analogous to adding 1 or 5 predictors?)

``` {r}
fit.exh.var <- sum_fit.gpm$which #indicators if variable is included
#pull vars for model.a, d=3
predictors <- colnames(fit.exh.var)[fit.exh.var[3,]] 
predictors[-c(1)] #drop intercept

#pull vars for model.b, d=4
colnames(fit.exh.var)[fit.exh.var[5,]]

#create models
model.a <- lm(gpm ~ horsepower + weight + year, auto_gpm)
model.b <- lm(gpm ~ horsepower + weight + year + cylinders, auto_gpm)

#compare and check for significance
