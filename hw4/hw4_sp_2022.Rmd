---
title: " Modern Data Mining, HW 4 - Group 18"
author:
- Group Member 1
- Group Member 2
- Group Member 3
...
Based on the mode fit to the randomly sampled 5 subjects, Liz's probability of having heart disease is 0.242. 
...
SE3 <- as.numeric(diff(confint.default(fit2, level = 0.99)[4,])/(2*2.57))
z1 <- summary(fit2)$coefficients[2,1]/SE1
z2 <- summary(fit2)$coefficients[3,1]/SE2
z3 <- summary(fit2)$coefficients[4,1]/SE3
p1 <- exp(-(0.717*z1) - (0.416*z1^2))
p2 <- exp(-(0.717*z2) - (0.416*z2^2))
p3 <- exp(-(0.717*z3) - (0.416*z3^2))
c(p1, p2, p3)

anova(fit2, test="Chisq") # Likelihood Ratio Test, via chi-square
```

Yes, all variables are significant at the 0.01 level. The p-values for the chi-squared test are as reported above (5.9e-13, 4.4e-05, 6.0e-11 for SBP, AGE, SEX respectively). The p-values for Wald's test are 7.73e-13, 9.07e-05, 4.41e-10 for SBP, AGE, SEX respectively. These are not the same across tests. 
...
Fit2 will always have a larger AUC than Fit1 because it has more variables. Our classifier can only be as or more accurate with more complexity (as long as Fit2 includes all the variables of Fit1 and more). Similarly, one curve will always contain the other, as it can only be more accurate the more complex it is. 
...
If we prioritize positive prediction, at a threshold of 0.5, we should go with fit1, as it has a slightly higher positive prediction. 
...
I would go with Fit2, in this case - as it can achieve a higher set of these values across thresholds. 
...
logit > log(0.0909/0.909)

ii. What is your estimated weighted misclassification error for this given risk ratio?

```{r}
final.model.bayes <- as.factor(ifelse(final.model$fitted > 0.0909, "1", "0"))
...
Since earlier we saw that our final model gave Liz a a 3.36% probability of being diagnosed with heart disease, this classifier would indicate that she does NOT fall into the HD category.
...
As we can see, under the assumption that a_10/a_01, the threshold given to us by Bayes rule does very well in finding the optimal threshold to minimize MCE.  
...
Now, the threshold given to us by the older Bayes rule assumptions does not do well in finding the optimal threshold to minimize MCE. This is of course because we've changed the assumptions of relative cost, so we would have to re-calculate the appropriate threshold under the new a_10/a_01 assumption. 
