...
pacman::p_load(dplyr, ggplot2, glmnet, car, data.table, skimr, GGally, ggpubr, tidyr, bestglm, pROC)   #add your packages here
```
...
Interestingly, race is not significant so we can remove it from the model.
...
The ROC curve and AUC clearly indicate that our final LASSO model is superior to the bestglm model. We'll proceed with prediction using `fit.logit.1` as our final model.

To start, we'll threshold our probability of readmission at 1/2.

```{r classify half}
#run classification
fit.pred.1 <- ifelse(fit.logit.1$fitted > 1/2, "1", "0") #y hat

#set.seed(10)
# output1 <- data.frame(data.train.lasso.m$readmit_30, fit.pred.1, fit.logit.1$fitted)[sample(57213, 15),] #set to 15 so we have at least 1 y=1

#compare readmission with prediction via confusion matrix - rows = predicted values, cols = actual
cm.1 <- table(fit.pred.1, data.train.lasso.m$readmit_30)
cm.1
```

Setting the threshold to 0.5 gives a sensitivity of _ and specificity of _. *to do: Fine-tune based on what seems to make sense with ROC curve*
...
Therefore we'll use Bayes thresholding on our final model.  *run on training&testing or validation data? Both?*

```{r Bayes rule}
# Applying Bayes rule
fit.logit.1.bayes <- as.factor(ifelse(fit.logit.1$fitted > .333, "1", "0"))
MCE.bayes <- (2 * sum(fit.logit.1.bayes[data.train.lasso.m$readmit_30 == "1"] != "1") 
                      + sum(fit.logit.1.bayes[data.train.lasso.m$readmit_30 == "0"] != "0"))/length(data.train.lasso.m$readmit_30)
MCE.bayes
```

*MCE.bayes looks pretty bad... to do: check sensitivity/specificty vs our predictions from part 2 and make some kind of comment about it*
