...
pacman::p_load(dplyr, ggplot2, glmnet, car, data.table, skimr, GGally, ggpubr)   #add your packages here
```
...
Read `readmission.csv` as `readmit_data`
```{r}
...
readmit_data <- readmit_data %>% mutate(encounter_id = as.factor(encounter_id),
                                        ...
                                        readmit_30 = as.factor(ifelse(readmitted == '<30', 1, 0))) #code new response var readmission indicator - tbh not sure if this should be factor
...
#NOTE: need to make clear in labels that readmit_30=0 could be readmission >30 days OR not readmitted!
...
Remove duplicate patient IDs such that we only have the last inpatient admission for each subject. *Note: Seems to make sense to take the latest hospitalization, since it's most "up-to-date" but we could take first instead? Either way it'll probably skew whether pts are readmitted or not, or we could try to filter randomly*

```{r}
#filter by unique pts's last hospitalization
readmit_latest <- readmit_data %>% 
  arrange(-number_inpatient) %>%
  filter(!duplicated(patient_nbr))
#filter by unique pts's first hospitalization
readmit_first <- readmit_data %>% 
  arrange(number_inpatient) %>%
  filter(!duplicated(patient_nbr))
#filter by unique pt's randomly
set.seed(100) #seed for reproducibility
readmit_rand <- readmit_data %>% 
   group_by(patient_nbr) %>%
   sample_n(1)

#quick plot to see differences between readmission among all data and unique pt subsets
p1 <- ggplot(readmit_data, aes(x=readmitted)) + 
  geom_bar(aes(y = (..count..)/sum(..count..))) + 
  ylim(0, 0.75) +
  labs(title = "Readmission in Full Dataset", y="Percent")
p2 <- ggplot(readmit_latest, aes(x=readmitted)) + 
  geom_bar(aes(y = (..count..)/sum(..count..))) + 
  ylim(0, 0.75) +
  labs(title = "Readmission in Latest Hosp Subset", y="Percent")
p3 <- ggplot(readmit_first, aes(x=readmitted)) + 
  geom_bar(aes(y = (..count..)/sum(..count..))) + 
  ylim(0, 0.75) +
  labs(title = "Readmission in First Hosp Subset", y="Percent")
p4 <- ggplot(readmit_rand, aes(x=readmitted)) + 
  geom_bar(aes(y = (..count..)/sum(..count..))) + 
  ylim(0, 0.75) +
  labs(title = "Readmission in Random Unique Subset", y="Percent")

ggarrange(p1, p2, p3, p4, ncol = 4, common.legend=T, labels = )
```
...
Trying LASSO random unique-pt sample - doesn't seem to work, matrix too big?!

```{r results=FALSE}
#remove columns we don't want to include as predictors or y
readmit_mod <- readmit_rand %>% dplyr::select(!c(encounter_id, patient_nbr, readmitted, max_glu_serum, A1Cresult))

Y <- readmit_mod[, 28] # extract Y
...
# names(fit.readmit.cv); summary(fit.readmit.cv)
# plot(fit.readmit.cv$lambda)
# fit.readmit.cv$cvm               # the mean cv error for each lambda 
# plot(fit.readmit.cv$lambda, fit.fl.cv$cvm, xlab="lambda", ylab="mean cv errors")
# fit.readmit.cv$lambda.min # lambda.min returns the min point all the cvm. 
# fit.readmit.cv$lambda.1se
# fit.readmit.cv$nzero     # string of number of non-zero coeff's returned for each lambda
```


Let's try subsetting a training sample and using backward selection
...
data.train <- readmit_rand[idx_train,]
data.test <- readmit_rand[idx_test,]
data.val <- readmit_rand[idx_val,]
``` 


``` {r}
#drop unnecessary cols
data.train.mod <- data.train %>% dplyr::select(!c(encounter_id, patient_nbr, readmitted, max_glu_serum, A1Cresult))
fit1 <- glm(readmit_30 ~ ., data.train.mod, family=binomial)  
```
