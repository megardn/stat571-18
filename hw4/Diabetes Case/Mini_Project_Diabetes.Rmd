...
In the U.S., diabetes is a chronic condition characterized by high blood sugar, leading to important health concerns. Diabetes is a major health care concern, as over 35 million (or 1 in 10) people are affected by this condition in the U.S. alone [CDC](https://www.cdc.gov/chronicdisease/programs-impact/pop/diabetes.htm). It also represents the nation's most expensive chronic condition, with over 237 billion dollars being spent annually on care for individuals with diabetes. Importantly, if diabetes is not detected/managed poorly, this can lead to serious health conditions and may cause the patient to undergo repeated hospital admissions, which in turn involves substantial costs. In addition, insurances no longer reimburse hospitals when patients are readmitted within 30 days, creating a strong incentive for hospitals to improve initial admission care and understand which factors may increase the risk of readmission within 30 days. In this study, we address this question using a public open dataset containing data collected from 70,000 patients with a total of 100,000 admissions over a period of 10 years (1999-2008) in 130 states, provided by the [Center for Clinical and Translational Research](https://archive.ics.uci.edu/ml/datasets/Diabetes+130-US+hospitals+for+years+1999-2008) at Virginia Commonwealth University. 

The goal of this study is to determine which factors significantly impact the risk of readmission within 30 days. To address this goal, we use two different methods, LASSO regression and AIC, to identify the most important risk factors for readmission within 30 days. We then provide a brief summary and visualization of findings, and finally will provide general conclusions and recommendations to improve care for diabetes and avoid short-interval readmission.
...
# plotting change in rx
readmit_uniq %>%
  drop_na(disch_disp_modified) %>% #drop NAs before plotting - makes sense i think?
  ggplot(aes(x = readmit_30)) + 
    geom_bar(aes(y = (..count..)/sum(..count..), fill = readmit_30)) + 
    facet_wrap(~disch_disp_modified) +
    labs(title = "Discharge location within 30 days by change in diabetes", y="Percent")
