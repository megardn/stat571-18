...
pacman::p_load(dplyr, ggplot2, glmnet, car, data.table, skimr, GGally, ggpubr, tidyr)   #add your packages here
```
...
```{r}
# Split the data - using unique pt admissions randomly sampled for now:
...
Let's use bestglm to find models with the smallest AIC:

```{r results=TRUE}
# Get the design matrix without 1's and HD
Xy_design <- model.matrix(readmit_30 ~.+0, data.train) #+0 -> no intercept, gets rid of first col with 1s
...
readmit_mod <- readmit_rand %>% dplyr::select(!c(encounter_id, readmitted))
...
I guess just try running one at a time?!

```{r}
fit1 <- glm(readmit_30~nb_admissions, readmit_mod, family=binomial(logit))  #summary(glm(as.numeric(HD)~SBP, fram_data, family=gaussian))
