...
#visualize dataframe
dim(brca)  #rows, columns
names(brca)[1:20]  # see a few variable names
summary(brca[, 1:3])  # only to peek the first 3 variables - lots of variability!
sum(is.na(brca))   # quick way to check missing values. use with caution - missing values may not be coded as "na"
```
...
```
    ...
    b) Randomly pick 5 genes and plot the histogram by each sub-type. **by subtype???**
...
brca %>% 
  ...
  ggtitle("Frequency of 5 Genes by Subtype") +
...
sel_cols <- which(colSums(abs(brca)) != 0) #list columns that aren't entirely 0's
brca_sub <- brca[, sel_cols, with=F] #keep only those columns in new df
dim(brca_sub) #now has ~300 fewer genes

#find columns with no variance
names(which(apply(brca_sub, 2, var) == 0)) #col name(s)
#no columns returned with no variance, nothing to remove

# log transform, adding 0.000000001 so that you're never trying to take log(0)
brca_sub <- log2(as.matrix(brca_sub+1e-10))

#replotting log-transformed values for fun
rand_genes <- colnames(select(brca, c(sample_idx))) #find names of genes randomly selected before
brca_subdf <- as.data.frame(brca_sub)
brca_subdf %>% 
  select(all_of(rand_genes)) %>%      # select same genes from previous figures
  pivot_longer(cols = everything()) %>%     
  ggplot(aes(x = value, y = ..density..)) +
  geom_histogram(aes(fill = name)) +
  ggtitle("Frequency of 5 Genes log-transformed by Subtype") +
  facet_wrap(~name, scales = "free") +
  theme_gray() +
  theme(legend.position = "none",)
```
...
brca_sub_kmeans <- kmeans(x = brca_sub, 4, nstart = 10)
