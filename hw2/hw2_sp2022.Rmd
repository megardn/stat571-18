...
pacman::p_load(ISLR, tidyverse, data.table, ggpubr, irlba, factoextra) # add the packages needed
```
...
basal_plot <- brca %>% 
  subset("BRCA_Subtype_PAM50" = "Basal") %>%  #select subtype
  select(all_of(sample_idx)) %>%      # select genes by column index
  pivot_longer(cols = everything()) %>% 
  ggplot(aes(x = value), color='black') +
  geom_histogram(aes(fill = name, position="identity"), alpha=0.6) +
  theme(legend.position = "none") +
  ggtitle("Basal")

her_plot <- brca %>% 
  subset("BRCA_Subtype_PAM50" = "Her2") %>%  #select subtype
  select(all_of(sample_idx)) %>%      # select genes by column index
  pivot_longer(cols = everything()) %>% 
  ggplot(aes(x = value)) +
  geom_histogram(aes(fill = name, position="identity"), alpha=0.6) +
...
luma_plot <- brca %>% 
  subset("BRCA_Subtype_PAM50" = "LumA") %>%  #select subtype
  select(all_of(sample_idx)) %>%      # select genes by column index
  pivot_longer(cols = everything()) %>% 
  ggplot(aes(x = value)) +
  geom_histogram(aes(fill = name, position="identity"), alpha=0.6) +
...
lumb_plot <- brca %>% 
  subset("BRCA_Subtype_PAM50" = "LumB") %>%  #select subtype
  select(all_of(sample_idx)) %>%      # select genes by column index
  pivot_longer(cols = everything()) %>% 
  ggplot(aes(x = value)) +
  geom_histogram(aes(fill = name, position="identity"), alpha=0.6) +
...
comp1 <- ggpubr::ggarrange(basal_plot, her_plot, luma_plot, lumb_plot, common.legend = T, legend = "bottom", ncol = 4)
annotate_figure(p=comp1, top = text_grob("5 Gene Frequencies by Cancer Subtype", 
                               color = "black", face = "bold", size = 20))

#alternate plotting with facet_grid
brca %>% 
...
plot(summary(pca_brca_test)$importance[3, ], pch=16,
     ylab="Cumulative PVE",
     xlab="Number of PC's",
     main="Scree plot of Cumulative PVE")

# run PCA via SVD, set 20 PCs
pca_brca <- prcomp_irlba(brca_sub, n=20, center = T, scale. = T)
...
We should use 4 PCs because the 4th is where the variability explained drops, but really not a lot of variance is explained overall as seen in the scree plot below.
...
```{r}
#looking at cumulative PVE for unscaled data to see how it compares
...
spec_table <- data.table(x = pca_brca_unscale$x[,1], #PC1
                y = pca_brca_unscale$x[,2], #PC2
...
kmeans_table <- data.table(clust = as.factor(brca_sub_kmeans$cluster), 
                spec.clust = as.factor(brca_spec$cluster), #spectrum cluster
...
kmeans_table %>%   ggplot() + 
  ...
  facet_wrap(~spec.clust, scales = "free") +
  xlab("Cluster Assigned by Spectrum Analysis")

#other plot
pairs(~ clust + spec.clust, data=kmeans_table, col=kmeans_table$type, size=count)

#x=ppt, y=pcs, color=subtype
```

There seems to be strong agreement between the clusters assigned by direct clustering and spectrum analysis.
